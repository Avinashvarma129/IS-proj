...
dataset = pd.read_csv('generated_points.csv')     #Imports the csv file containing the data
...
def plot_solution(coords, solution):
    ...
    for idx in range(n - 1):
        i, next_i = int(solution[idx]), int(solution[idx + 1])
        plt.plot(
    ...
    i, next_i = int(solution[-1]), int(solution[0])
    plt.plot(
        [coords[i, 0], coords[next_i, 0]],
        [coords[i, 1], coords[next_i, 1]],
        c="blue",
    ...
    plt.text(coords[int(solution[0]), 0], coords[int(solution[0]), 1], "Start", ha="right", va="bottom")
    plt.text(
        coords[int(solution[-1]), 0], coords[int(solution[-1]), 1], "End", ha="right", va="bottom"
    )
    plt.show()
...
def main(dataset, T, cooling_rate, T_lower_bound, tolerance):
    ...
    plot_solution(cities, current_solution) 
